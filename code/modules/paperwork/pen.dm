...
/obj/item/weapon/sigildrawer/attack_self(mob/user)
	var/turf/T
	if(used == FALSE)
		name = "Nar-Sie heart"
		desc = "A heretical weapon."
		icon_state = "narsie"
		used = TRUE
		T = get_turf(user)
		new /obj/effect/decal/cleanable/sigil(T)
		visible_message("<font color=purple>[user] drawed a sigil!</font>")
	else
		visible_message("<font color=purple>[user] tried to draw something but failed.</font>")

/obj/effect/decal/cleanable/sigil
	name = "sigil"
	density = 0
	layer = 2
	icon = 'icons/obj/sigil.dmi'
	icon_state = "sigil"
	anchored = 1
	pixel_w = 12
	pixel_y = 15